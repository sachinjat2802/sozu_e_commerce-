...
// Define a new event handler for deleting an item by ID
export default defineEventHandler(async (event, res) => {
  ...
  if (event.method === 'DELETE') {
    try {
      ...
      // Check if the item exists in the database by ID
    ...
    // Send a response with a 405 Method Not Allowed status code for non-DELETE requests
    return {
      statusCode: 405,
